document.addEventListener('DOMContentLoaded', function() {
    ...
    // Reset login state if user didn't just log in
    ...
    // Update navigation based on final login state
    updateNavigation(localStorage.getItem('isLoggedIn') === 'true');
    ...
    function updateNavigation(isLoggedIn) {
        const joinNowBtn = document.getElementById('btn-join');
        const joinNowLinks = document.querySelectorAll('.login-link');
        
        if (isLoggedIn && joinNowBtn) {
            // Replace JOIN NOW button with profile icon and dropdown
            const navLinks = document.querySelector('.nav-links');
            const liElement = joinNowBtn.parentElement;
            ...
            profileLi.innerHTML = `
                ...
                <div class="dropdown-menu">
                    <ul>
                        ...
                        <li><a href="#"><i class="fas fa-id-card"></i> Membership</a></li>
                        <li><a href="#"><i class="fas fa-dumbbell"></i> My Classes</a></li>
                        <li><a href="#"><i class="fas fa-shopping-bag"></i> Order History</a></li>
                        <li><a href="#" id="logout-btn"><i class="fas fa-sign-out-alt"></i> Logout</a></li>
                    </ul>
                </div>
            `;
            
            // Replace JOIN NOW button with profile icon
            if (liElement) {
                liElement.replaceWith(profileLi);
            }
            
            // Update all other login links to profile links
            joinNowLinks.forEach(link => {
                const parentElement = link.parentElement;
                const profileLink = document.createElement('a');
                profileLink.href = '#';
                profileLink.className = 'profile-link btn-primary';
                profileLink.innerHTML = '<i class="fas fa-user-circle"></i> My Profile';
                
                if (parentElement) {
                    parentElement.replaceChild(profileLink, link);
                }
            });
    ...
    window.calculateBMI = function() {
        ...
        if (usingMetric) {
            height = document.getElementById('height').value / 100; // Convert cm to meters
            weight = document.getElementById('weight').value;
        } else {
            height = (document.getElementById('height').value * 0.3048); // Convert feet to meters
            weight = (document.getElementById('weight').value * 0.453592); // Convert pounds to kg
        }
        ...
        if (height > 0 && weight > 0) {
            ...
            resultDiv.innerHTML = `Your BMI is <strong>${bmi}</strong> <br><span class="${categoryClass}">(${category})</span>`;
    ...
    window.switchUnits = function() {
        ...
        if (usingMetric) {
            ...
            switchButton.innerText = "Switch to Feet & Pounds";
            
            // Convert values if they exist
            if (heightInput.value) heightInput.value = (heightInput.value * 30.48).toFixed(1);
            if (weightInput.value) weightInput.value = (weightInput.value * 0.453592).toFixed(1);
        } else {
            ...
            switchButton.innerText = "Switch to cm & kg";
    ...
    // Add event listener for btn-join button
...
function initializeCustomCursor() {
    ...
    const interactiveElements = document.querySelectorAll('a, button, .btn, .forge-text, .membership-card, .btn-join, .filter-btn, .close-modal, #cartToggle, #nightModeToggle, .cart-item-remove, .footer-legal a, .footer-links a');
    ...
    interactiveElements.forEach(element => {
        ...
        element.addEventListener('mouseenter', () => {
            if (!element.closest('.footer-legal') && !element.closest('.footer-links')) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Configuration
    const SCROLL_DURATION = 2000; // 3.5 seconds for smooth animation
